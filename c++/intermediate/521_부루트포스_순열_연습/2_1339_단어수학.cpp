...
int calc(const vector<int> &order) {
    int sum = 0;
    int ten = 1;
...
int main() {
    ...
    for (int i = 0; i != N; ++i) {
        ...
        for (auto c : words[i]) {
            ...
            mapTable[c - 'A'] = ++numAlpha;
        }
        reverse(words[i].begin(), words[i].end());
        maxLen = max(maxLen, words[i].size());
    }

    vector<int> choose(10);
    for (int i = 0; i != numAlpha; ++i) {
        choose[9 - i] = 1;
    }

    vector<int> order(numAlpha);
    int ret = 0;
    do {
        int idx = 0;
        for (int i = 0; i != 10; ++i) {
