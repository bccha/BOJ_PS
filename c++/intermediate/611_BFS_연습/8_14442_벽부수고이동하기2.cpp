...
void bfs() {
    ...
    while (!q.empty()) {
        ...
        if (cur.first.first == N - 1 && cur.first.second == M - 1) {
            cout << visited[cur.second][cur.first.first][cur.first.second] << endl;
            return;
        }
        q.pop_front();
        FOR (i, 4) {
            ...
            if (!visited[cur.second][ny][nx] && B[ny][nx] != '1') {
                visited[cur.second][ny][nx] = visited[cur.second][cur.first.first][cur.first.second] + 1;
                q.push_back({{ny, nx}, cur.second});
            }

            if (cur.second < K && !visited[cur.second + 1][ny][nx] && B[ny][nx] == '1') {
                visited[cur.second + 1][ny][nx] = visited[cur.second][cur.first.first][cur.first.second] + 1;
