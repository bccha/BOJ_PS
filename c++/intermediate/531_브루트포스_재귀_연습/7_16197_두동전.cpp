...
int dfs(int cur) {
    ...
    if (coin[0].first == coin[1].first && coin[0].second == coin[1].second) {
        return ret;
    }
    for (int i = 0; i != 2; ++i) {
        if (coin[i].first < 0 || coin[i].first >= N || coin[i].second < 0 || coin[i].second >= M) {
            return ret;
        }
    }
    ...
    for (int i = 0; i != 4; ++i) {
        for (int j = 0; j != 2; ++j) {
            coin[j].first += dyx[i][0];
            coin[j].second += dyx[i][1];
        }
        ret = min(dfs(cur + 1), ret);
        for (int j = 0; j != 2; ++j) {
            coin[j].first -= dyx[i][0];
            coin[j].second -= dyx[i][1];
        }
