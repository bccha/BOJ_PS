...
void combination(string &op, int k) {
    vector<int> C;
    C.resize(op.length(), 0);
    for (int i = k; i != op.length(); ++i) {
        C[i] = 1;
    }
    int minVal = 2'000'000'000;
    int maxVal = -2'000'000'000;
    do {
        string subOp;
        ...
        } while (next_permutation(subOp.begin(), subOp.end()));
